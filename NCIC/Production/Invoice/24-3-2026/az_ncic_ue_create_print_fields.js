 ...
 */
define(["./az_ats_amount_in_words", 'N/ui/serverWidget', 'N/record','N/search', 'N/file', "N/query"], function (Amount_converter, serverWidget, record, search,file, query) {
    ...
    const beforeLoad = (context) => {
        try {
            ...
            if (context.type === context.UserEventType.PRINT) {
                ...
                const lchargeID = Record.getSublistValue({
                    sublistId: 'item',
                    fieldId: 'custcol_ino_pms_lease_charge',
                    line: 0
                });
                const leaseChargeNum= getLeaseCharge(lchargeID);
                setleaseCharge(context,leaseChargeNum);
            }
        } catch (errBeforeLoad) {
   ...
   const getLeaseCharge = (lchargeID) => {
        try {
            if (!lchargeID) return '';
            ...
            const lookup = search.lookupFields({
                ...
                id: lchargeID,
                columns: ['custrecord_ino_pms_lc_number']
            });

            let lcNumber = '';

            if (lookup.custrecord_ino_pms_lc_number) {
                lcNumber = lookup.custrecord_ino_pms_lc_number;
            }

            return lcNumber;

        } catch (errorGetLeaseCharge) {
            log.debug("errorGetLeaseCharge", errorGetLeaseCharge);
    ...
    const setleaseCharge = (context, leaseChargeNum) => {

        const leaseCharge = context.form.addField({
            id: 'custpage_lease_charge',
            type: serverWidget.FieldType.TEXT,
            label: 'Lease Charge'
        });

        leaseCharge.defaultValue = leaseChargeNum;
    }
