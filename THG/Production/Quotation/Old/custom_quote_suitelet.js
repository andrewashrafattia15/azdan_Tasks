function printQuoteAction(request, response) {
  try {
    ...
    template +=
      '<td style ="width:90%;">:${record.custbody_payment_terms}</td>';
    template += "</tr>";
    template += "</#if>";

    template += "<#if record.custbody_bank_details?has_content>";
    template += "<tr>";
    template += '<td style ="width:10%;">&nbsp;</td>';
    template += '<td style ="width:90%;">${record.custbody_bank_details}</td>';
    template += "</tr>";
    template += "</#if>";
    //
    ...
    template +=
      '<td style="align: center; font-weight: bold;">AUTHORIZED SIGNATORY<br/><#if record.custbody_signature?string=="Yes"><img src="http://4659918.shop.netsuite.com/core/media/media.nl?id=11018&amp;c=4659918&amp;h=XNeS0lTY6Yqq1zkMtEvd5cZhfiR98kLRBQfvXo3KiRadUeW9" width="110" height="75"/></#if></td></tr>';
    template +=
