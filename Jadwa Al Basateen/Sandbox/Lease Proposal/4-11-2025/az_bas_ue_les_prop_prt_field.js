...
define(['N/search', 'N/ui/serverWidget'], function (search, serverWidget) {
    ...
    const beforeLoad = (context) => {
        try {
            ...
            if (context.type === context.UserEventType.PRINT) {
                ...
                const {numOfElevators,parkingSpaces,buildingNum,units} = getUnitsElevators(unitsIDs);
                setData(numOfElevators,parkingSpaces,buildingNum,grandRent,ratedRent,units,context);
            }
            ...
            if (unitsIds && unitsIds.length > 0) {
                ...
                if (searchResult && searchResult.length > 0) {
                    for (let i = 0; i < searchResult.length; i++) {
                        ...
                        units.push(searchResult[i].getValue('custrecord_ino_re_um_number'));
                    }
                }
            }

            return {numOfElevators,parkingSpaces,buildingNum,units} ;
    ...
    const setData = ( numOfElevators,parkingSpaces,buildingNum ,grandRent,ratedRent,units,context) => {
