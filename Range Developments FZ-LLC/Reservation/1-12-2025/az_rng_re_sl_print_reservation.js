...
define(['N/search', 'N/render', 'N/record', 'N/file'], function (s, render, record, file) {
            ...
            if (context.request.method == 'GET') {
                try {
                    ...
                    if (myRecId) {
                        ...
                        let vendorData = getVendorData(subsidiaryId);
    ...
    const getBody = (template, reservationData, customerData, vendorData, paymentPlanData, subLogo, propertyData, unitData, partnerData, secondCustomerData, thirdCustomerData, fourthCustomerData,fifthCustomerData, subsidiaryId, accNumber, IBAN, accName) => {
        try {
            ...
            // // table 5 (PROJECT ESCROW ACCOUNT DETAILS title / Account Bank Details (AED) title)
            {
                if(reservationData.bankAccount == "Escrow"){
                    template += '<table style="width:100%;">';
                ...
                else if(reservationData.bankAccount == "Corporate"){
                    template += '<table style="width:100%;">';

                    template += '<tr>';
                    template += '<td  style=" margin-bottom:15px;"><span style="border-bottom:1px solid black;"><b>Account Bank Details (AED)</b></span></td>';
                    template += '</tr>';
            ...
            // // table 6 (PROJECT ESCROW ACCOUNT DETAILS / Account Bank Details (AED))
            {
                if(reservationData.bankAccount == "Escrow"){
                    ...
                    template += '<tr>';
                    template += '<td width="35%"><b>Account Name</b></td>';
                    template += '<td width="65%">: ' + vendorData.legalname + '</td>';
                    template += '</tr>';

                    template += '<tr>';
                    template += '<td><b>IBAN</b></td>';
                    template += '<td>: ' + vendorData.iban + '</td>';
                    template += '</tr>';

                    template += '<tr>';
                    template += '<td><b>Account Number</b></td>';
                    template += '<td>: ' + vendorData.accountNumber + '</td>';
                    template += '</tr>';
                    template += '<tr>';
                    template += '<td><b>Bank Name, Branch and Address</b></td>';
                    template += '<td>: ' + 'Bank of Sharjah' + '</td>';
                    template += '</tr>';

                    template += '<tr>';
                    template += '<td><b>SWIFT code</b></td>';
                    template += '<td>: ' + vendorData.swiftCode + '</td>';
                    template += '</tr>';
            ...
            // // table 12 (Payment Schedule)
            {
                ...
                if (reservationData.subsidiary == 2||reservationData.subsidiary == 3||reservationData.subsidiary == 8) {
                ...
                for (i = 0; i < paymentPlanData.res.length; i++) {
                    if (paymentPlanData.res[i].isInActive == false) {
                        ...
                        if ( reservationData.subsidiary == 8) {
                                const cutoffDate = new Date('2027-12-31'); 
                                const oldDate = new Date(paymentPlanData.res[i].oldDate);
                                const newDate = new Date(paymentPlanData.res[i].date);
                                let remark = (oldDate > cutoffDate || newDate > cutoffDate) ? remarkNote : '-';
                                if(paymentPlanData.res[i].type == 2){
    ...
    const getReservationData = (myRecId) => {
        try {
            if (myRecId) {
                ...
                let bankAccount = opportunityRec.getText('custbody_az_rng_bank_account_type');
        ...
        try {
            if (subsidary) {
                ...
                let subsidiaryRec = s.lookupFields({
                    type: 'subsidiary',
                    id: subsidary,
                    columns: ['legalname', 'name', 'email','custrecord_az_rng_iban','custrecord_az_rng_account_number','custrecord_az_rng_swift_code']
                });
                ...
                return {
                    ...
                    'address': address,
                    'regNum': regNum,
                    'iban': subsidiaryRec.custrecord_az_rng_iban,
                    'accountNumber': subsidiaryRec.custrecord_az_rng_account_number,
                    'swiftCode': subsidiaryRec.custrecord_az_rng_swift_code
                }
            } else {
                return {
                    'name': "",
