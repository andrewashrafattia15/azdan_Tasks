    ...
    try {
      ...
      // // THIS AGREEMENT
      {
        ...
        if (customerData != null && customerData != '') {
          template += `<li style="padding-bottom: 5px;">${escapeXml(customerData.custname)}, ${vowelWord(customerData.nationality)} national,` 

          if(customerData.custType == true) {
            template += `with passport number ${escapeXml(customerData.passportno)} whose address is at ${escapeXml(customerData.custaddress)} <strong>(“the Purchasers”)</strong> AND</li>`;
          }
          else{
            template += `with registration number ${escapeXml(customerData.compRegNum)} whose address is at ${escapeXml(customerData.custaddress)} <strong>(“the Purchasers”)</strong> AND</li>`;
          }
        }
 ...
 const getCustomerData = (custID)=>{
    try {
      ...
      if (custSearch != null && custSearch != "") {
        ...
        const compRegNum = custSearch[0].getValue("custentity_az_rng_company_reg_num");
