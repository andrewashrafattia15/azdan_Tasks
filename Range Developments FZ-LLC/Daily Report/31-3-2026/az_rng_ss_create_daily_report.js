...
define(['N/file', 'N/render', 'N/search', 'N/format', 'N/record','N/runtime'], function (file, render, search, format, record,runtime) {
    ...
    const execute = (context) => {
        try {
            ...
            if (dayNumber == 0) {
                ...
                const tBHConsolidatedCashBalance = getCashBalance(2,endDate)
                const tBRConsolidatedCashBalance = getCashBalance(3,endDate)
                const tBVConsolidatedCashBalance = getCashBalance(8,endDate)
                const tBlConsolidatedCashBalance = getCashBalance(9,endDate)
                
                // consolidated report file
                consolidatedTemplate = getConsolidatedHeader(endDate,startDate)
                consolidatedTemplate = getConsolidatedBody(consolidatedTemplate, tBHConsolidatedData, tBHConsolidatedTotal, tBRConsolidatedData, tBRConsolidatedTotal, tBVConsolidatedData, tBVConsolidatedTotal, tBHConsolidatedCashBalance, tBRConsolidatedCashBalance, tBVConsolidatedCashBalance, tBlConsolidatedCashBalance, endDate, tBlConsolidatedData, tBlConsolidatedTotal)
                consolidatedFile = savePdfFile(consolidatedTemplate, 'Consolidated_Reports_', 88363, endDate)
            }
    ...
    const getConsolidatedHeader = (endDate,startDate) => {
        try {
            ...
            template += '<macrolist>'
            template += '<macro id="nlheader">'
            template += `<p align="center">Consolidated Report from ${startDate} to ${endDate}</p>`
            template += '</macro>'
            template += '<macro id="nllastheader">'
            template += `<p align="center">Cash Balance as of ${endDate}</p>`
            template += '</macro>'
            template += '<macro id="nlfooter">'
            template += '</macro>'
            template += '</macrolist>'
    ...
    const getConsolidatedBody = (template, tBHData, tBHTotal, tBRData, tBRTotal, tBVData, tBVTotal, tBHBalance, tBRBalance, tBVBalance, tBlBalance, date, tBlData, tBlTotal) => {
        try {
            //table 1 (Beach House)
            {
